...
abstract class CModel extends CComponent implements ArrayAccess
{
	...
	public function createValidators()
	{
		...
		foreach($this->rules() as $rule)
		{
			if(isset($rule[0],$rule[1]))  // attributes, validator name
				...
				throw new CException(get_class($this).' has an invalid validation rule. The rule must specify attributes to be validated and the validator name.');
		}
		return $validators;
	}
	...
	{
		//if(DEBUG)
			//Yii::log(strFormat('Failed to set unsafe attribute "{attribute}" of "{class}".',array('{attribute}'=>$name, '{class}'=>get_class($this))),CLogger::LEVEL_WARNING);
	}
