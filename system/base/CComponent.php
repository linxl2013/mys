...
class CComponent
{
	...
	public function __call($name,$parameters)
	{
		...
		throw new CException(get_class($this).' and its behaviors do not have a method or closure named "'.$name.'".');
	}
	...
	public function attachBehavior($name,$behavior)
	{
		if(!($behavior instanceof IBehavior))
			...
			$behavior=createComponent($behavior);
	...
	public function getEventHandlers($name)
	{
		if($this->hasEvent($name))
		{
			...
			throw new CException('Event "'.get_class($this).'.'.$name.'" is not defined.');
	}
				...
				elseif(is_callable($handler,true))
				{
					if(is_array($handler))
					{
						...
						elseif(method_exists($object,$method))
							...
							throw new CException('Event "'.get_class($this).'.'.$name.'" is attached with an invalid handler "'.$handler[1].'".');
					}
					else // PHP 5.3: anonymous function
						call_user_func($handler,$event);
				}
				else
					throw new CException('Event "'.get_class($this).'.'.$name.'" is attached with an invalid handler "'.gettype($handler).'".');
				// stop further handling if param.handled is set true
				if(($event instanceof CEvent) && $event->handled)
					return;
			}
		}
		elseif(DEBUG && !$this->hasEvent($name))
			throw new CException('Event "'.get_class($this).'.'.$name.'" is not defined.');
	}
