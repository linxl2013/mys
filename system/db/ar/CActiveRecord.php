...
abstract class CActiveRecord extends CModel
{
	...
	public function getRelated($name,$refresh=false,$params=array())
	{
		...
		if(!isset($md->relations[$name]))
			throw new CDbException(strFormat('{class} does not have relation "{name}".',
	...
	public function getDbConnection()
	{
		...
		else
		{
			...
			if(self::$db instanceof CDbConnection)
				...
				throw new CDbException(strFormat('Active Record requires a "db" CDbConnection application component.'));
		}
	}
	...
	public function insert($attributes=null)
	{
		if(!$this->getIsNewRecord())
			throw new CDbException(strFormat('The active record cannot be inserted to database because it is not new.'));
		if($this->beforeSave())
		{
	...
	{
		if($this->getIsNewRecord())
			throw new CDbException(strFormat('The active record cannot be updated because it is new.'));
		if($this->beforeSave())
		{
	...
	public function delete()
	{
		if(!$this->getIsNewRecord())
		{
			...
			throw new CDbException(strFormat('The active record cannot be deleted because it is new.'));
	}
...
class CManyManyRelation extends CHasManyRelation
{
	...
	private function initJunctionData()
	{
		if(!preg_match('/^\s*(.*?)\((.*)\)\s*$/',$this->foreignKey,$matches))
			throw new CDbException(strFormat('The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key. The format of the foreign key must be "joinTable(fk1,fk2,...)".',
...
class CActiveRecordMetaData
{
	...
	public function __construct($model)
	{
		...
		if(($table=$model->getDbConnection()->getSchema()->getTable($tableName))===null)
			throw new CDbException(strFormat('The table "{table}" for active record class "{class}" cannot be found in the database.',
	...
	public function addRelation($name,$config)
	{
		if(isset($config[0],$config[1],$config[2]))  // relation class, AR class, FK
			...
			throw new CDbException(strFormat('Active record "{class}" has an invalid configuration for relation "{relation}". It must specify the relation type, the related active record class and the foreign key.', array('{class}'=>get_class($this->_model),'{relation}'=>$name)));
	}
