...
class CDbCommand extends CComponent
{
	...
	public function prepare()
	{
		if($this->_statement==null)
		{
			...
			catch(Exception $e)
			{
				...
				throw new CDbException('CDbCommand failed to prepare the SQL statement: '.$e->getMessage(),(int)$e->getCode(),$errorInfo);
			}
		}
	}
	...
	public function execute($params=array())
	{
		...
		catch(Exception $e)
		{
			...
			//Yii::log(strFormat('CDbCommand::execute() failed: {error}. The SQL statement executed was: {sql}.',
			//	array('{error}'=>$message, '{sql}'=>$this->getText().$par)),CLogger::LEVEL_ERROR,'system.db.CDbCommand');

			if(DEBUG)
				$message.='. The SQL statement executed was: '.$this->getText().$par;

			throw new CDbException('CDbCommand failed to execute the SQL statement: '.$message,(int)$e->getCode(),$errorInfo);
		}
	}
	...
	private function queryInternal($method,$mode,$params=array())
	{
		...
		catch(Exception $e)
		{
			...
			$errorInfo=$e instanceof PDOException ? $e->errorInfo : null;
			$message=$e->getMessage();
			//Yii::log(strFormat('CDbCommand::{method}() failed: {error}. The SQL statement executed was: {sql}.',
			//	array('{method}'=>$method, '{error}'=>$message, '{sql}'=>$this->getText().$par)),CLogger::LEVEL_ERROR,'system.db.CDbCommand');

			if(DEBUG)
				$message.='. The SQL statement executed was: '.$this->getText().$par;

			/*throw new CDbException(strFormat('CDbCommand failed to execute the SQL statement: {error}',
				array('{error}'=>$message)),(int)$e->getCode(),$errorInfo);*/
			throw new CDbException('CDbCommand failed to execute the SQL statement: '. $message, $e->getCode(), $errorInfo);
		}
	}
	...
	private function processConditions($conditions)
	{
		...
		throw new CDbException('Unknown operator "'.$operator.'".');
	}
