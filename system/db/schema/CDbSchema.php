...
abstract class CDbSchema extends CComponent
{
	...
	public function getTable($name,$refresh=false)
	{
		...
		else
		{
			...
			if(!isset($this->_cacheExclude[$name]) && ($duration=$this->_connection->schemaCachingDuration)>0 && $this->_connection->schemaCacheID!==false && ($cache=getComponent($this->_connection->schemaCacheID))!==null)
			{
				$key='mys:dbschema'.$this->_connection->connectionString.':'.$this->_connection->username.':'.$name;
				$table=$cache->get($key);
	...
	{
		throw new CDbException(strFormat('{class} does not support fetching all table names.',
