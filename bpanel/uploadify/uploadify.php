...
if (!empty($_FILES)) {
	...
	$targetPath = ROOT_PATH ."/". $targetFolder;
	// Validate the file type
	$fileTypes = array('jpg','jpeg','gif','png','rar','docx','doc','pptx','ppt','xlsx','xls','pdf','txt'); // File extensions
	$fileParts = pathinfo($_FILES['Filedata']['name']);
	if (in_array($fileParts['extension'] = strtolower($fileParts['extension']),$fileTypes)) {
		switch ($_FILES['Filedata']['error']) {
			case UPLOAD_ERR_OK :
				$randName = time()."_".rand(10,99);
				clearstatcache();
				...
				$targetFile =  rtrim($targetPath,'/') . '/' .$randName. '.' . $fileParts['extension'];
				move_uploaded_file($tempFile,$targetFile);
				if(!empty($_POST["createThumb"])){
					$thumbName = rtrim($targetPath,'/') . '/' .$randName. '_thumb.' . $fileParts['extension'];
					createThumb($targetFile, $thumbName,'', 67, 80);
				}
				$result['save_name']  = $randName;
